...
> **页头版本号**：标题右侧显示当前 skill 版本（取自 `manifest.yaml` 的 `version`），两种模式都显示，用于快速确认跑的是哪一版。

**分面板加载与局部刷新**（v1.4.0）：工作台拆成 `概览` / `消耗明细` / `包生命周期` 三个面板，各自请求一个接口、各自渲染。
顶部「面板」状态条显示每个面板的耗时与更新时间：点面板名跳到该面板，点 `↻` **只刷新这一个面板**（只重取它自己的数据源）。
服务端对应接口：`/api/overview`、`/api/requests`、`/api/lifecycle`（`?fresh=1` 强制重取该面板拥有的源），另有 `/api/version`（瞬时，不打上游）。
四路服务端数据**并发**拉取 + 进程内缓存（single-flight）去重，冷启动约 1 秒、后续刷新约 0.3 秒；任一源失败只影响对应面板，其余照常渲染。

> **服务版本自检**（v1.4.1）：常驻服务**不重启不加载新代码**，而旧进程会一直占着 8090 —— 启动器曾因此误判「已在运行」，每次都打开旧版页面（症状是新前端打不到新接口，三个面板齐报红）。现在启动器会先探 `/api/version` 与 `manifest.yaml` 的版本比对，不一致就结束旧进程再拉起新版（只对 python 系进程动手；端口被别的程序占用时只提示、不误杀）。页面侧同样先探一次后端能力：服务若为旧版，会**自动降级为全量模式**并给出提示与「重新检测」按钮，而非让面板报红。
...
> **容量口径：订阅型用「本周期」而非「累计」（v1.4.2）**
> 接口对每条资源**同时**返回两套容量字段：累计 `CapacitySize/Used/Remain` 与周期 `CycleCapacitySize/Used/Remain`。
> 对**订阅型（`CapacityType == 4`，即「套餐用量」）必须取周期口径** —— 实测服务端累计字段对这类资源
> **不随周期消耗更新**（同一对象 `CapacityUsed=0` 而 `CycleCapacityUsed=500`；而周期区间是累计区间的子集，
> 累计已用不可能小于周期已用），会把「已用光」的套餐算成满额，令「可用积分」系统性高估。官网控制台
> 按周期口径渲染（其「下次权益周期更新时间」= `CycleEndTime`），工作台现已对齐。其余类型（赠送包/加量包）
> 两组数值一致，仍用累计口径。页面检测到分叉时会标注「已按本周期口径修正 X」，批次表对订阅型行标「· 本周期」。

> **「今日已使用」的口径（v1.4.3）**：一律 **L5 服务端流水优先、逐包采样兜底**，且与 `usage_source` 的标注保持一致。
> 逐包采样（L2）只统计「今日**首个采样点之后**」的增量 —— 采样点由页面刷新/服务请求产生，当天第一次打开
> 工作台之前的消耗**没有基准**，会系统性低估（实测 2026-09-16：流水 9.66 被算成 0.74）。故 L2 只作兜底，
> 不作主口径；`usage_source` 为 L2 时页面会如实标注来源，对账状态也会报 `no_baseline`。
>
